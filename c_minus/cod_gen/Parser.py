...
def params():
    ...
    if token == TokenType.VOID:
        # Save position to check if void is followed by ID
        saved_token = token
        ...
        # Check if this is void as parameter type (void ID) or just void
        if token == TokenType.ID:
            # It's a void parameter, restore and parse as param-list
            # Actually, void parameters are not allowed in C-
            syntaxError("Los parametros no pueden ser de tipo void")
            return params_list
        else:
            # It's just void (no parameters)
            return params_list
